...
def update(request):
    ...
    # NOTE: These methods are a pretty bad way to do things if I ever wanted to expand this application out past
    # the United States Congress. For instance, adding an if statement case for every party in the UK parliament
    # would probably be a bad idea. Instead I would precompile a manual list of all of the parties and keep that curated
    # so that when I found an abbreviation or title for a party I could match it without trouble. THEORETICALLY
    def find_legislative_body(legislator_name):
        """ A little helper method for querying the LegislativeBody's existing object set """
        if legislator_name[:3] == 'Rep':
            return LegislativeBody.objects.get_or_create(abbreviation='HR',
                                                         name='House of Representatives',
                                                         title='Rep')[0]
        elif legislator_name[:3] == 'Sen':
            return LegislativeBody.objects.get_or_create(abbreviation='S',
                                                         name='Senate',
                                                         title='Sen')[0]
        else:
            raise ValueError('Unknown legislative body or badly formatted name: {n}'.format(n=legislator_name))
    ...
    def populate_bill(url, b=None, name=None, last_modified_string=None):
        ...
        if not b:
            ...
            if not created and b.last_modified == last_modified_date:
                logging.info('Found existing bill {number} to return'.format(title=b.bill_number))
                return b
        ...
        try:
            ...
            raise KeyError('Error parsing billCommittees from {url}'.format(url=url))
        try:
        ...
        for sponsor in sponsors:
            ...
            # legislative_body = find_legislative_body(full_name)
            if is_senator:
                ...
                legislative_body = LegislativeBody.objects.get_or_create(name='House of Representatives',
                                                                         abbreviation='HR')
                district = District.objects.get_or_create(number=district_number, state=state)
                legislator = Representative.objects.get_or_create(lis_id=lis_id, party=party, state=state,
                                                                  first_name=first_name, last_name=last_name,
                                                                  legislative_body=legislative_body, district=district)
            # legislator = Legislator.objects.get_or_create(lis_id=lis_id, party=party, state=state,
            #                                               first_name=first_name, last_name=last_name,
        ...
        for co_sponsor in co_sponsors:
            ...
            legislator = Legislator.objects.get_or_create(lis_id=lis_id, party=party, legislative_body=legislative_body,
                                                          state=state, first_name=first_name, last_name=last_name)[0]
        ...
        for action in actions:
            try:
                ...
                committee_string = action['committee']
    ...
    # to be safe.
    url = 'https://www.govinfo.gov/bulkdata/json/BILLSTATUS/115/s'
    ...
    if response.status != 200:
        logging.warning('Error fetching {url): {status}'.format(url=url, status=response.status))
        return
    ...
    for file in response_data['files']:
        ...
        bill_status_name = parse_without_coercion('justFileName', file, url)
        last_modified_string = parse_without_coercion('formattedLastModifiedTime', file, url)
        populate_bill(url=bill_status_url, name=bill_status_name, last_modified_string=last_modified_string)
